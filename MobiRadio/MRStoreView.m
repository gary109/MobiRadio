...
- (void) updateAllFrame {
    [self.gydelegate updateSettingViewFrame];
    [self.gydelegate updateBatteryImage];
}
...
- (void) handleBtnClicked:(id)sender
{
    if(_TAG_STORE_BACK_BTN_ == ((UIButton *)sender).tag) {
        [self updateAllFrame];
        [self.gydelegate hiddenStoreView];
...
//                    UIAlertView * myAlertView = [[UIAlertView alloc] initWithTitle:@"Please waiting ..."
//                                                                           message:nil
//                                                                          delegate:self
    ...
    for (SKPaymentTransaction *transaction in queue.transactions)
    {
        ...
        if([productID isEqualToString:IAP_RemoveAds_PRODUCT_ID]) {
            [MRPlist writePlist:@"Setting"
                         forkey:@"RemoveAds"
                        content:[NSString stringWithFormat:@"%@", @"1"]];
            [self updateAll];
            [self updateAllFrame];
            [self.gydelegate removeAdmob];
        }
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        if(!contentCreated) {
            ...
            //[uiTitleView setTag:_TABLETITLEVIEW_TAG_];
...
- (void) updateRemoveAds {
    ...
    if ([removeAds isEqualToString:@"0"]){
        ...
        [MRPlist writePlist:@"Setting"
                     forkey:@"RemoveAds"
                    content:[NSString stringWithFormat:@"%@", @"0"]];
    }
}
- (void) updateAll {
